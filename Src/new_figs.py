...
#   sns.scatterplot(x='mean_real', y='mean_shuf', data=res.sort_values(by='sig'), hue='sig', ax=ax[2], alpha=0.5, palette=col2[:4])
...
#   s7_sorted = create_maximally_mixed_scales(s7)
...
#   ax[2].set_xticklabels(['', 250, 500, 750, ''])
    ...
    for a in ax[:]:
        try:
            ...
            pass
#   ax[6].set_yticks(np.arange(0, 1.2, .2))
...
#   regplot(np.log10(prob_obs[idx1]), np.log10(prob_obs[idx3]), ax[1], (10**-12, 10**-2), sns.color_palette()[1])
...
#   print(pearsonr(np.log10(prob_obs[idx1]), np.log10(prob_obs[idx2])))
#   print(pearsonr(np.log10(prob_obs[idx2]), np.log10(prob_obs[idx3])))
#   print(pearsonr(np.log10(prob_obs[idx1]), np.log10(prob_obs[idx3])))

#   print(pearsonr(prob_less[idx1]>0.5, prob_less[idx2]>0.5))
#   print(pearsonr(prob_less[idx1]>0.5, prob_less[idx3]>0.5))
