...
PATH_RAW = Path("/home/johnmcbride/projects/Scales/DaMuSc")

PATH_BASE = [p for p in [Path.cwd()] + list(Path.cwd().parents) if p.name == 'DaMuSc_Paper_Code'][0]
...
def process_theory_scales(use_all_variants=False, use_compound=False):
    df = pd.read_csv(PATH_RAW.joinpath("Data", 'theory_scales.csv'))
    cols_to_keep = ['TheoryID', 'Name', 'Culture', 'Region', 'Country', 'SocID', 'Tuning', 'RefID']
...
def process_measured_scales(oct_cut=50, use_specific_variants=True, use_all_variants=False):
    df = pd.read_csv(PATH_RAW.joinpath("Data", 'measured_scales.csv'))
    df.Intervals = df.Intervals.apply(utils.str_to_ints)
    cols_to_keep = ['MeasuredID', 'Name', 'Culture', 'Region', 'Country', 'SocID', 'Measured_type', 'Inst_type', 'RefID']
    new_cols = ['ScaleID'] + ['n_notes', 'scale', 'step_intervals', 'tonic_intervals', 'all_intervals'] + cols_to_keep
    df_new = pd.DataFrame(columns=new_cols)
...
def instrument_tunings():
    df = pd.read_csv(PATH_RAW.joinpath("Data", 'measured_scales.csv'))
    df.Intervals = df.Intervals.apply(utils.str_to_ints)
    df["scale"] = df.Intervals.apply(lambda x: [0] + list(np.cumsum(x)))
    df["n_notes"] = df.Intervals.apply(len)
    ...
    df['irange'] = df['max_int'] - df['min_int']
    df['largest_int'] = df.scale.apply(lambda x: x[-1])
    df['second_largest_int'] = df.scale.apply(lambda x: max(x[-2], x[-1] - x[1]))
    return df
